      ...
      sections: [
        ...
        },
        {
          ...
          items: [
            ...
            'McpChatService: agentic loop using claude-sonnet-4 with up to 10 tool-call rounds; ALLOWED_TOOLS curation gate (15 read-only tools) prevents context window overflow; sanitizeResponse strips all leaked tool narration tag variants',
            'POST /v1/mcp/chat: JWT + caseworker/admin role required, 120s timeout with structured timeout/503 error responses, MCP_ENABLED guard',
            'System prompt conventions: latestVersion=true for listing/counting, maxResults=20 for listing, dedicated count tools for counting, no tool narration',
            ...
            'McpChatSection: chat bubble UI, typing indicator, Enter-to-send, Clear chat button (only visible when messages exist), backend error message propagation',
            'EPIPE errors from MCP child process stdio suppressed — no longer crash the backend on disconnect',
            'Dashboard layout fixed: h-screen + min-h-0 on flex containers eliminates page-level scrollbar; overflow-y-auto on message area conditioned on messages.length > 0 eliminates empty scrollbar track on Windows',
